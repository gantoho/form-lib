class Form {
  ...
  _createCustomSelect(field, wrapper) {
    ...
    if (triggerType !== 'submit') {
      // 使用 mousedown + 组合方式来检测失焦，更可靠
      document.addEventListener('mousedown', (event) => {
        // 如果点击的是当前 select 外部的元素
        if (!container.contains(event.target) && container.classList.contains('select--was-focused')) {
          container.classList.remove('select--was-focused');
          this._validateField(field, wrapper);
        }
      });
      ...
      container.addEventListener('focusin', () => {
        container.classList.add('select--was-focused');
      });

      // 保留 focusout 作为备用（处理 Tab 键切换等场景）
      container.addEventListener('focusout', (event) => {
        // 延迟检查，确保 relatedTarget 已更新
        setTimeout(() => {
