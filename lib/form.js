...
const DEFAULT_ICONS = {
  error: '<svg viewBox="0 0 16 16"><path fill="currentColor" d="M8 0a8 8 0 100 16A8 8 0 008 0z"/><path fill="#fff" d="M5.354 5.354a.5.5 0 11.708-.708L8 7.293l1.938-1.647a.5.5 0 01.708.708L8.707 8l1.942 1.937a.5.5 0 01-.708.708L8 8.707l-1.938 1.938a.5.5 0 01-.708-.708L7.293 8z"/></svg>',
  info: '<svg viewBox="0 0 16 16"><path fill="currentColor" d="M8 0a8 8 0 100 16A8 8 0 008 0zm.5 3h-1v1h1V3zM8 5.5a.75.75 0 01.53 1.28L7.5 8.3a.75.75 0 00-.22.53V10h2v-.6a.7.7 0 01.2-.5l1.1-1.1a.75.75 0 000-1.06L8.5 5.4A.75.75 0 008 5.5z"/></svg>',
  'password-on': '<svg viewBox="0 0 16 16" fill="currentColor"><path d="M8 3C4.6 3 1.7 5.2 1 8c.7 2.8 3.6 5 7 5s6.3-2.2 7-5c-.7-2.8-3.6-5-7-5zm0 3a2 2 0 110 4 2 2 0 010-4zm0 1a1 1 0 100 2 1 1 0 000-2z"/></svg>',
  'password-off': '<svg viewBox="0 0 16 16" fill="currentColor"><path d="M8 3C4.6 3 1.7 5.2 1 8c.3 1.1 1 2 1.9 2.8L2.2 12.5a.5.5 0 00.7.7l1.8-1.8A6.5 6.5 0 008 11c3.4 0 6.3-1.8 7 .7V15h-1V8 5.7l3.1-3.1a.5.5 0 00-.7-.7zM5.5 9.6L7.3 7.8A2 2 0 019.8 6l-1 1a2 2 0 01-1.1 2.1l-1.6 1.6a.5.5 0 01-.7-.7z"/></svg>',
  clear: '<svg viewBox="0 0 16 16"><path fill="currentColor" d="M8 0a8 8 0 100 16A8 8 0 008 0z"/><path fill="#fff" d="M5.354 5.354a.5.5 0 11.708-.708L8 7.293l1.938-1.647a.5.5 0 01.708.708L8.707 8l1.942 1.937a.5.5 0 01-.708.708L8 8.707l-1.938 1.938a.5.5 0 01-.708-.708L7.293 8z"/></svg>',
  arrow: '<svg viewBox="0 0 16 16" fill="none"><path d="M4 6l4 4 4-4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>',
};
...
class Form {
  ...
  _resolveIconHTML(conf, fallback) {
    if (conf === true || conf == null) return fallback;
    if (typeof conf === 'string') return conf;
    if (conf.svg) return conf.svg;
    if (conf.src) return '<img src="' + conf.src + '" alt="" />';
    if (conf.image) return '<img src="' + conf.image + '" alt="" />';
    return fallback;
  }
  ...
  _makePasswordToggle(control) {
    ...
    const render = () => {
      ...
      el.innerHTML = show ? DEFAULT_ICONS['password-on'] : DEFAULT_ICONS['password-off'];
